...
export default function MissionDetailPage() {
  ...
  async function handleSendDailyComment(e: React.FormEvent) {
    e.preventDefault();
    if (!participation || !dailyComment.trim()) return;
    setSendingDailyComment(true);
    try {
      await feedbackApi.create({
        participationId: participation.id,
        note: dailyRating,
        commentaires: dailyComment.trim(),
                    ...
                    {isDayValidated ? (
                      <>
                        ...
                        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/80 p-4">
                          ...
                          <p className="mt-1 text-[11px] text-slate-500">
                            Un bug rencontré, une difficulté ou une remarque sur l&apos;application testée lors de ce Jour {selectedEtape.ordre} ? Ce retour est facultatif mais précieux pour l&apos;équipe.
                          </p>
                          ...
                          {dailyCommentSent ? (
                            ...
                            <form onSubmit={handleSendDailyComment} className="mt-3 space-y-2.5">
                              <div className="flex items-center gap-2">
                                <span className="text-[11px] font-medium text-slate-600">Votre ressenti du jour :</span>
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-[11px] font-bold text-slate-600 ml-1">{dailyRating}/5</span>
                                </div>
                              </div>
                              ...
                              <textarea
                                ...
                                className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs text-navy-900 placeholder:text-slate-400 focus:border-brand-orange focus:outline-none focus:ring-1 focus:ring-brand-orange resize-none"
                              />

                              <div className="flex justify-end">
                                <button
                                  type="submit"
                                  disabled={sendingDailyComment || !dailyComment.trim()}
                                  className="inline-flex items-center gap-1.5 rounded-xl bg-navy-900 px-3.5 py-1.5 text-xs font-bold text-white hover:bg-navy-800 transition disabled:opacity-40"
                                >
                                  {sendingDailyComment ? (
                                    <>
                                    ...
                                    <>
                                      <Send size={12} />
                                      <span>Envoyer mon avis (optionnel)</span>
                                    </>
                                  )}
                                </button>
                              </div>
                      ...
                      <form onSubmit={handleValidateViaSdk} className="mt-4 space-y-4">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                          ...
                          <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3 focus-within:border-brand-orange focus-within:bg-white focus-within:ring-1 focus-within:ring-brand-orange transition">
                            <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1">
                              Identifiant Testeur
                            </label>
                            <div className="flex items-center gap-2">
                              <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-200/60 text-slate-600 shrink-0">
                                <Users size={14} />
                              </span>
                              <input
                                type="text"
                                ...
                                className="w-full bg-transparent font-mono text-xs sm:text-sm font-bold text-navy-900 placeholder:text-slate-400 focus:outline-none"
                              />
                            </div>
                          ...
                          <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-3 focus-within:border-brand-orange focus-within:bg-white focus-within:ring-1 focus-within:ring-brand-orange transition">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                className="w-full bg-transparent font-mono text-xs sm:text-sm font-bold text-navy-900 placeholder:text-slate-400 focus:outline-none uppercase"
                              />
                              {currentReference?.reference && simCode !== currentReference.reference && (
                                <button
                                  type="button"
                                  onClick={() => setSimCode(currentReference.reference)}
                                  className="shrink-0 rounded-lg bg-amber-100 px-2 py-1 text-[10px] font-bold text-amber-800 hover:bg-amber-200 transition"
                        ...
                        <button
                          ...
                          className="flex w-full items-center justify-center gap-2 rounded-xl bg-brand-orange py-2.5 sm:py-3 text-xs font-bold text-white shadow-xs transition hover:bg-orange-600 disabled:opacity-50 active:scale-98"
                        >
                          {simLoading ? (
                            <>
