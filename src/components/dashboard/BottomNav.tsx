...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-3 inset-x-3 max-w-md mx-auto z-40 lg:hidden">
      <div className="grid grid-cols-4 items-center rounded-[26px] border border-slate-200/80 bg-white/95 px-2 py-2 shadow-[0_16px_36px_rgba(15,23,42,0.16)] backdrop-blur-xl">
        {navItems.map((item) => {
            ...
            >
              <div
                className={`relative flex h-8 w-8 items-center justify-center rounded-xl transition-all duration-200 ${
                    item.active ? "bg-navy-900 text-white scale-110 shadow-md shadow-navy-900/20" : ""
                }`}
              >
